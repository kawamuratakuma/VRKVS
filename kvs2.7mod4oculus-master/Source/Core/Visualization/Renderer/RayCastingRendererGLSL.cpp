...
kvs::AnyValueArray NormalizeValues(const kvs::StructuredVolumeObject* volume,
        const kvs::Real32 min_value, const kvs::Real32 max_value) {
...
RayCastingRenderer::RayCastingRenderer() :
        m_draw_front_face(true), m_draw_back_face(true), m_draw_volume(true), m_enable_jittering(
                false), m_step(0.5f), m_opaque(1.0f) {
 ...
 */
/*===========================================================================*/
void RayCastingRenderer::exec(kvs::ObjectBase* object, kvs::Camera* camera,
        kvs::Light* light) {
    BaseClass::startTimer();
    kvs::StructuredVolumeObject* volume = kvs::StructuredVolumeObject::DownCast(
            object);
    kvs::OpenGL::WithPushedAttrib p( GL_ALL_ATTRIB_BITS);

//    std::cerr << "#" << std::endl;
//    std::cerr << "###1 " << camera->windowWidth() << " : "
//            << camera->windowHeight() << std::endl;

//    {
//        int viewport[4];
//        kvs::OpenGL::GetViewport(viewport);
//        std::cerr << "###1 " << viewport[0] << " : " << viewport[1] << " : "
...
//    }
    ...
    if (BaseClass::windowWidth() == 0 && BaseClass::windowHeight() == 0) {
        ...
        this->initialize_framebuffer(camera->windowWidth(),
                camera->windowHeight());
    }
...
//    std::cerr << "###2 " << BaseClass::windowWidth() << " : "
//            << BaseClass::windowHeight() << std::endl;
    ...
    // OpenGL variables.
    const kvs::Mat4 PM = kvs::OpenGL::ProjectionMatrix()
            * kvs::OpenGL::ModelViewMatrix();
    const kvs::Mat4 PM_inverse = PM.inverted();

//    std::cerr << "###3 " << PM << std::endl;
//    std::cerr << "###3.1 " << camera->projectionModelViewMatrix() << std::endl;
//    std::cerr << "###3.2 " << kvs::OpenGL::ProjectionMatrix() << std::endl;
//    std::cerr << "###3.3 " << kvs::OpenGL::ModelViewMatrix() << std::endl;
    ...
    {
        kvs::FrameBufferObject::GuardedBinder bind(m_entry_exit_framebuffer);
        if (m_draw_back_face) {
            // Draw the back face of the bounding cube for the entry points.
            kvs::OpenGL::SetDrawBuffer( GL_COLOR_ATTACHMENT0_EXT);
            kvs::OpenGL::Clear( GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    ...
    if (m_draw_volume) {
        ...
        m_ray_casting_shader.bind();
        {
            ...
            m_ray_casting_shader.setUniform("ModelViewProjectionMatrixInverse",
                    PM_inverse);
            ...
            const kvs::Vec3 light_position = kvs::WorldCoordinate(
                    light->position()).toObjectCoordinate(object).position();
            const kvs::Vec3 camera_position = kvs::WorldCoordinate(
                    camera->position()).toObjectCoordinate(object).position();
 ...
 */
/*===========================================================================*/
void RayCastingRenderer::setDrawingBuffer(
        const RayCastingRenderer::DrawingBuffer drawing_buffer) {
    m_draw_front_face = false;
    m_draw_back_face = false;
    m_draw_volume = false;

    switch (drawing_buffer) {
    case RayCastingRenderer::FrontFace:
        m_draw_front_face = true;
        break;
    case RayCastingRenderer::BackFace:
 ...
 */
/*===========================================================================*/
void RayCastingRenderer::setTransferFunction(
 ...
 */
/*==========================================================================*/
void RayCastingRenderer::initialize_shader(
...
#endif
        if (m_enable_jittering)
            frag.define("ENABLE_JITTERING");
        if (BaseClass::isEnabledShading()) {
    ...
    const kvs::Real32 max_ngrids = static_cast<kvs::Real32>(kvs::Math::Max(
            r.x(), r.y(), r.z()));
    const kvs::Vec3 resolution(static_cast<float>(r.x()),
            static_cast<float>(r.y()), static_cast<float>(r.z()));
    const kvs::Vec3 ratio(r.x() / max_ngrids, r.y() / max_ngrids,
            r.z() / max_ngrids);
    ...
    if (type == typeid(kvs::UInt8)) {
    ...
    } else if (type == typeid(kvs::UInt32) || type == typeid(kvs::Int32)
            || type == typeid(kvs::Real32) || type == typeid(kvs::Real64)) {
        min_range = 0.0f;
        max_range = 1.0f;
        min_value = 0.0f;
        max_value = 1.0f;
        ...
        kvsMessageError(
                "Not supported data type '%s'.", volume->values().typeInfo()->typeName());
    }
...
void RayCastingRenderer::initialize_jittering_texture() {
    ...
    for (size_t i = 0; i < size * size; i++)
        rnd[i] = static_cast<kvs::UInt8>(255.0f * rand() / float(RAND_MAX));
    ...
    m_jittering_texture.setPixelFormat( GL_LUMINANCE8, GL_LUMINANCE,
    GL_UNSIGNED_BYTE);
 ...
 */
/*===========================================================================*/
void RayCastingRenderer::initialize_bounding_cube_buffer(
        const kvs::StructuredVolumeObject* volume) {
    ...
    const float coords[nelements] = { minx, miny, minz, // 0
            maxx, miny, minz, // 1
            maxx, miny, maxz, // 2
            minx, miny, maxz, // 3
...
void RayCastingRenderer::initialize_transfer_function_texture() {
    ...
    const kvs::ValueArray<kvs::Real32> table =
            BaseClass::transferFunction().table();
    ...
    m_transfer_function_texture.setPixelFormat( GL_RGBA32F_ARB, GL_RGBA,
    GL_FLOAT);
 ...
 */
/*===========================================================================*/
void RayCastingRenderer::initialize_volume_texture(
        const kvs::StructuredVolumeObject* volume) {
...
//            << std::endl;
    ...
    if (type == typeid(kvs::UInt8)) {
        ...
        data_value = ::NormalizeValues<kvs::UInt32>(volume, min_value,
                max_value);
        ...
        data_value = ::NormalizeValues<kvs::Int32>(volume, min_value,
                max_value);
        ...
        data_value = ::NormalizeValues<kvs::Real32>(volume, min_value,
                max_value);
        ...
        data_value = ::NormalizeValues<kvs::Real64>(volume, min_value,
                max_value);
    } else {
 ...
 */
/*===========================================================================*/
void RayCastingRenderer::initialize_framebuffer(const size_t width,
        const size_t height) {
    ...
    m_depth_texture.setPixelFormat( GL_DEPTH_COMPONENT, GL_DEPTH_COMPONENT,
    GL_FLOAT);
 ...
 */
/*===========================================================================*/
void RayCastingRenderer::update_framebuffer(const size_t width,
        const size_t height) {
...
void RayCastingRenderer::draw_bounding_cube_buffer() {
    ...
    KVS_GL_CALL( glColor4f(0.0f, 0.0f, 1.0f, 1.0f ) );KVS_GL_CALL( glEnableClientState( GL_VERTEX_ARRAY ) );KVS_GL_CALL( glVertexPointer( 3, GL_FLOAT, 0, 0 ) );KVS_GL_CALL( glDrawArrays( GL_QUADS, 0, 72 ) );KVS_GL_CALL( glDisableClientState( GL_VERTEX_ARRAY ) );
}
...
void RayCastingRenderer::draw_quad(const float opacity) {
    ...
    p1.loadIdentity();
    {
        ...
        p2.loadIdentity();
        {
            ...
            KVS_GL_CALL_BEG( glBegin( GL_QUADS ) );KVS_GL_CALL_VER( glColor4f( 1.0, 1.0, 1.0, opacity ) );KVS_GL_CALL_VER( glTexCoord2f( 1, 1 ) );KVS_GL_CALL_VER( glVertex2f( 1, 1 ) );KVS_GL_CALL_VER( glTexCoord2f( 0, 1 ) );KVS_GL_CALL_VER( glVertex2f( 0, 1 ) );KVS_GL_CALL_VER( glTexCoord2f( 0, 0 ) );KVS_GL_CALL_VER( glVertex2f( 0, 0 ) );KVS_GL_CALL_VER( glTexCoord2f( 1, 0 ) );KVS_GL_CALL_VER( glVertex2f( 1, 0 ) );KVS_GL_CALL_END( glEnd() );
        }
    }
}
