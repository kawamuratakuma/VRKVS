...
{
#ifdef DEBUG_HUD
    std::cerr << "HMD::initialize_hud_texture()!!" << std::endl;
#endif // DEBUG_HUD
    ...
    if (!hudTexture->ColorTextureChain || !hudTexture->DepthTextureChain)
    {
        //        return false;
        return false;
    }
    ...
    return m_widget_handler->initialize();
}
...
bool HeadMountedDisplay::initialize_mirror_texture()
{
    ...
    result = ovr_CreateMirrorTextureWithOptionsGL(m_handler, &desc, &m_mirror_tex);
    if (!OVR_SUCCESS(result)) {
        //if (retryCreate) goto Done;
        kvsMessageError("Failed to create mirror texture.");
    }

    // Configure the mirror read buffer
    GLuint texId;
    ovr_GetMirrorTextureBufferGL(m_handler, m_mirror_tex, &texId);
    ...
    ovr_SetTrackingOriginType(m_handler, ovrTrackingOrigin_FloorLevel);
